...
// Tuned for 0.1 CPU / 512 MB RAM — smaller buffers to reduce memory pressure & CPU overhead
const MUSIC_CONNECTION_TIMEOUT_MS = parsePositiveInt(process.env.MUSIC_CONNECTION_TIMEOUT_MS, 45_000);
const MUSIC_BUFFERING_TIMEOUT_MS = parsePositiveInt(process.env.MUSIC_BUFFERING_TIMEOUT_MS, 12_000);
const MUSIC_PREBUFFER_BYTES = parsePositiveInt(process.env.MUSIC_PREBUFFER_BYTES, 256 * 1024);
const MUSIC_PREBUFFER_MAX_WAIT_MS = parsePositiveInt(process.env.MUSIC_PREBUFFER_MAX_WAIT_MS, 4_000);
...
function normalizeQuery(rawQuery) {
    ...
    try {
        ...
        if (YOUTUBE_HOSTS.has(host)) {
            ...
            // Watch page (regular YouTube or YouTube Music)
            ...
            // YouTube Music specific: /channel, /browse, or other paths with a list param
            if (host === 'music.youtube.com' && playlistId) {
